...
func (wscli *WebSocketClient) Subscribe(nodeIndex int, filter EventFilter, eventHandler WsEventHandler) (SubscriptionID, StdError) {
	...
	jsonReq.Params[0] = filter.GetEventType()
	jsonReq.Params[1] = filter.Serialize()
	...
	wscli.conns[nodeIndex].subscriptions =
		append(wscli.conns[nodeIndex].subscriptions, Subscription{Event: filter.GetEventType(), SubscriptionID: subID})
