...
int get_public_key(unsigned char bytes[65])
{
      EC_KEY* p_key = EC_KEY_new_group();
      EC_KEY_set_conv_form(p_key,POINT_CONVERSION_COMPRESSED);

      int size = i2o_ECPublicKey(p_key,NULL);
      if(!size||size>65)
         return 0;
...
// return NULL on fail
BIGNUM* calcu_y(const EC_GROUP* group, const BIGNUM* x, BIGNUM *y/*=NULL*/, BN_CTX* ctx/*=NULL*/) {
    if (!y) y = BN_new();
    char ctx_cf = 0;
    if (!ctx) ctx = BN_CTX_new(), ctx_cf = 1;
    EC_POINT* pnt = EC_POINT_new(group);
    xck(EC_POINT_set_compressed_coordinates_GFp(group, pnt, x, 1, ctx));
    xck(EC_POINT_get_affine_coordinates_GFp(group, pnt, NULL, y, ctx));
    if (ctx_cf) BN_CTX_free(ctx);
    return y;
}

#define BN_dump(bn,arr) memset(arr, 0, 32); BN_bn2bin(bn, (unsigned char*)(arr+32)-BN_num_bytes(bn))
void SM2_calcy_interface(const unsigned char* x_bin, unsigned char* y_bin) {
    BIGNUM* x = BN_bin2bn(x_bin, 32, NULL);
    BIGNUM* y = calcu_y(sm2_ec_group(), x, NULL, NULL);
    BN_dump(y, y_bin);
    BN_free(x), BN_free(y);
}

// return NULL on fail
BIGNUM* calcu_y_0(const EC_GROUP* group, const BIGNUM* x, BIGNUM *y/*=NULL*/, BN_CTX* ctx/*=NULL*/) {
    if (!y) y = BN_new();
    char ctx_cf = 0;
    if (!ctx) ctx = BN_CTX_new(), ctx_cf = 1;
    EC_POINT* pnt = EC_POINT_new(group);
    xck(EC_POINT_set_compressed_coordinates_GFp(group, pnt, x, 0, ctx));
    xck(EC_POINT_get_affine_coordinates_GFp(group, pnt, NULL, y, ctx));
    if (ctx_cf) BN_CTX_free(ctx);
    return y;
}

void SM2_calcy_interface_0(const unsigned char* x_bin, unsigned char* y_bin) {
    BIGNUM* x = BN_bin2bn(x_bin, 32, NULL);
    BIGNUM* y = calcu_y_0(sm2_ec_group(), x, NULL, NULL);
    BN_dump(y, y_bin);
    BN_free(x), BN_free(y);
}