...
func (parser *Parser) parseField(field *ast.Field) *structField {
	...
	descTag := reflect.StructTag(structTag).Get("desc")
	if descTag != "" {
		structField.descValue = defineTypeOfDesc(structField.schemaType, descTag)
	}
...
// defineTypeOfDesc return desc string
func defineTypeOfDesc(schemaType string, descValue string) interface{} {
	return descValue
}

// GetAllGoFileInfo gets all Go source files information for given searchDir.
func (parser *Parser) getAllGoFileInfo(searchDir string) {
	Walk(searchDir, parser.visit)
}
